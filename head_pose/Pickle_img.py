...
"""
Use this script to produce RGB images and corresponding height maps using the Intel Realsense Camera.
Saves the RGB image as a .png and saves the height map as a .pkl file.
"""
...
try:
    ...
    while True:
        ...
        if key == ord('k'):
            rgb_name = f"jestinp_{save_index}_-15roll_rgb.png"
            pickle_name = f"jestinp_{save_index}_-15roll_height.pkl"

            # Save RGB image (BGR)
            cv2.imwrite(rgb_name, color_image)

            # Create height map (meters). Mark invalid depth (0) as NaN.kkkkkkkkkkkkk
